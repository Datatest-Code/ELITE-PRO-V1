...
await EliteProEmpire.sendMessage(from, {text: xeonlod[i], edit: key });
        ...
        if (autobio) {
            EliteProEmpire.updateProfileStatus(`єхplσríng thє frσntíєrs σf αrtífícíαl íntєllígєncє, tσgєthєr!. User: ${ownername}`).catch(_ => _)
        }
...
let onlineUsers = participants.filter(p => p.isOnline)
if (onlineUsers.length === 0) return reply('❌ No one is online right now.')
